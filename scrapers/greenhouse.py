...
def extract_relevant_description(text):
    ...
    low_priority_description_indices = (
        []
    )  # positions of excerpts that are lower case (could be mid-sentence or not desc section starting points)
    for kw in keywords:
        idx = text.lower().find(kw)
        if idx != -1:
            # only if idx is NOT within 200 chars of ANY gathered excerpts idx pos'ns
            if not any(
                abs(idx - pos) < 200 for pos in final_excerpts_indices_to_texts.keys()
            ):

                if (
                    len(final_excerpts_indices_to_texts) < 2
                ):  # still have room for more excerpts?
                    current_excerpt = text[
                        idx : idx + 1100
                    ]  # slice original text, not text.lower() to maintain casing
                    ...
                    low_priority_description_indices and current_excerpt[0].isupper()
                ):  # there is any low priority excerpt index and current excerpt is upper case
        ...
        if (
            len(final_excerpts_indices_to_texts) == 2
            and not low_priority_description_indices
        ):
            break
    final_excerpts_texts = list(final_excerpts_indices_to_texts.values())
    if (
        len(final_excerpts_texts) == 2
        and final_excerpts_texts[0][:200].strip()
        == final_excerpts_texts[1][:200].strip()
    ):
...
def fetch_jobs_by_company(slug):
    ...
    try:
        ...
        for j in jobs:
            ...
            relevant_description_text = extract_relevant_description(
                description_text_without_html
            )
...
def get_all_gh_jobs_filtered():
    ...
    relevant_jobs = [
        j for j in all_jobs if not any(kw in j["title"].lower() for kw in all_bad)
    ]
    usa_relevant_jobs = [
        j for j in relevant_jobs if "united states" in j["location"].lower()
    ]
